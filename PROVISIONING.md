# How to provision a new environment for terceroAPP
...
* Enter domains to secure (for example): `tercer-afrodita`
* Enter webroot path: `/home/deployer/apps/terceroAPP/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/tercer-afrodita/fullchain.pem /etc/ssl/terceroAPP.crt
ln -s /etc/letsencrypt/live/tercer-afrodita/privkey.pem /etc/ssl/terceroAPP.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/terceroAPP/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
7 7,19 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/terceroAPP/shared/public/
```
